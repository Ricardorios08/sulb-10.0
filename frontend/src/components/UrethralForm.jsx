...
const UrethralForm = ({ initialData, onChange }) => {
  ...
  return (
    <div className="bg-white p-2 border-0">
      <div className="alert alert-info py-2 mb-3 d-flex align-items-center gap-2">
        <FaFlask />
        <span className="fw-bold text-uppercase small">Examen Uretral (903)</span>
      </div>
      ...
      <div className="row g-2">
        ...
        <div className="col-12 mt-2">
          <label className="form-label small fw-bold text-muted">Nota</label>
          <input type="text" className="form-control form-control-sm" value={data.nota || ''} onChange={(e) => handleChange('nota', e.target.value)} />
        </div>
      </div>
    </div>
  );
