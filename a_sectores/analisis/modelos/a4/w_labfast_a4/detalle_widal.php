<?php 
//$pdf->AddPage();

$pdf->Ln();

$sql11="select * from detalle_widal where cod_grabacion = $cod_grabacion and nro_practica = $nro_practica";
$result11 = $db->Execute($sql11);


$flagelares=$result11->fields["flagelares"];
$somatico=$result11->fields["somatico"];
$paratyphi_a=$result11->fields["paratyphi_a"];
$paratyphi_b=$result11->fields["paratyphi_b"];
$observaciones=$result11->fields["observaciones"];



 $sql11="select * from convenio_practica where cod_practica =  $nro_practica";
$result11 = $db->Execute($sql11);


$nombre_practica=strtoupper($result11->fields["practica"]);
$unidad=$result11->fields["unidad"];


$pdf->SetFont('Arial','BIU',10);
$pdf->Cell(60,10,'REACCION DE WIDAL');
$pdf->SetFont('Arial','',9);
$pdf->ln();
$pdf->ln();

 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(80,10,"RESULTADO:",0); 
 $pdf->ln();
 $pdf->ln();

 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(60,10,"Ac. Flagelares (H): ",0);     
  $pdf->SetFont('Arial','B',$tamanio);
$pdf->Cell(30,10,$flagelares,0);
 $pdf->SetFont('Arial','',$tamanio);
 $pdf->ln();
 $pdf->ln();


 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(60,10,"Ac. Somáticos (O): ",0);     
  $pdf->SetFont('Arial','B',$tamanio);
$pdf->Cell(30,10,$somatico,0);
 $pdf->SetFont('Arial','',$tamanio);
 $pdf->ln();
 $pdf->ln();

 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(60,10,"Ac. Paratyphi A: ",0);     
  $pdf->SetFont('Arial','B',$tamanio);
$pdf->Cell(30,10,$paratyphi_a,0);
 $pdf->SetFont('Arial','',$tamanio);
 $pdf->ln();
  $pdf->ln();

 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(60,10,"Ac. Paratyphi B: ",0);     
  $pdf->SetFont('Arial','B',$tamanio);
$pdf->Cell(30,10,$paratyphi_b,0);
 $pdf->SetFont('Arial','',$tamanio);
 $pdf->ln();
 $pdf->ln();
  $pdf->ln();


 $pdf->SetFont('Arial','U',$tamanio);
$pdf->Cell(60,10,"Nota: ",0);     
  $pdf->SetFont('Arial','B',$tamanio);
$pdf->Cell(100,10,$observaciones,0);
 $pdf->SetFont('Arial','',$tamanio);

  

?>